...
static TimeStamp BuildTimeStamp(String s) throws UOMoRuntimeException{
	 ...
	 if (sp > -1) {
		 ...
		 if (tz == null) throw new UOMoRuntimeException("Time Zone not found: " + zoneString);  
 ...
 * @throws BDTypeException
 */
static TimeStamp BuildTimeStamp(java.sql.Timestamp ts) throws UOMoRuntimeException{
...
public String formatWithZone(String s) throws UOMoRuntimeException {
	  ...
	  if (s.charAt(0) == '+' || s.charAt(0) == '-') {
		  ...
		  ts.setTime(millis);
	  	  ...
	  	  if (tz == null) throw new UOMoRuntimeException("Time Zone not found: " + s);
	      millis += tz.getRawOffset();
	      ts.setTime(millis);
	      if (tz.inDaylightTime(ts)) 
